...
e il \texttt{ComponentsContainer} descritto precedentemente al Paragrafo~\ref{sec:container-di-componenti}.
...
Tuttavia, questo ha comportato ad alcuni problemi come l'impossibilità di garantire a tempo di compilazione che le tuple
...
Per questo, prendendo ispirazione dall'approccio utilizzato nella libreria Shapeless~\ref{TODO}, ho individuato
una possibile soluzione nell'adozione delle \texttt{CList}, realizzate in seguito da Nicolas con il mio aiuto.
...
Infatti, sarebbe stato inaccettabile per un utente dover specificare manualmente i \texttt{ComponentTag}
...
Tramite un semplice import (\texttt{usign dev.atedeg.ecscala.given}) l'utente non deve preoccuparsi
della presenza dei \texttt{ComponentTag} che saranno generati automaticamente dal compilatore tramite
un'apposita macro riportata nel Listato~\ref{TODO}.
...
Per esempio, come viene mostrato nel Listato~\ref{TODO}, non è possibile specificare una lista di
componenti con tipi ripetuti nella definizione di un sistema in quanto questo non avrebbe
senso dato che un'entità può avere al più un componente per tipo.

L'implementazione di tali controlli è riportata al Listato~\ref{TODO}.
Si può osservare come venga sfruttato

\subsubsection{Container di Componenti}

\subsubsection{Deletable CLists}
...
Durante l'implementazione si è però posto il problema di poter specificare di rimuovere un particolare componente
dall'entità: per esempio, il sistema di un videogioco che opera su entità che hanno un componente \texttt{LifePoints}
...
(\eg \texttt{Some(<updated-component>) \&: None \&: CNil}) avrebbe comportato una riscrittura delle \texttt{CList}
in modo tale da poter accettare valori opzionali.

Una soluzione, a mio avviso più elegante, ha sfruttato due nuovi meccanismi introdotti in Scala 3:
i \textit{match types}\cite{TODO} e gli \textit{union types}\cite{TODO}.
Come mostrato nel Listato~\ref{TODO} una versione ''deletable'' di una \texttt{CList}
\texttt{T1 \&: ... \&: TN \&: CNil} non è altro che la lista stessa dove ogni elemento viene sostituito da
uno union type \texttt{(T1 | Deleted)}.
Dato che il tipo \texttt{T} è sottotipo di \texttt{T | Other} una \texttt{CList} sarà sottotipo della sua
...
\texttt{update} dei sistemi; una sostanziale differenza sta nel fatto che, al posto di un qualunque componente
potrà specificare lo speciale componente \texttt{Deleted} ad indicare che il componente in tale posizione è
