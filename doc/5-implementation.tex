...
Mi sono occupato della realizzazione delle~\texttt{CList}, garantendo aspetti di \textit{type safety}
affinché eventuali errori nel loro utilizzo vengano intercettati a compile-time.
La peculiarità di questa lista è che gli elementi in essa contenuti possono essere di tipi differenti: questa
costruzione prende il nome di~\textit{heterogeneous list}.
La costruzione delle \texttt{CList} è ispirata alla libreria shapeless~\cite{shapeless}.
...
sotto tipo di~\Component, al tal proposito si è specificato l'\textit{upper-type bound} per rispettare questo vincolo.

Oltre all'implementazione, mi sono occupato anche dello sviluppo dei test delle~\texttt{CList}: un testing approfondito
si è reputato fondamentale poiché questo costrutto rappresenta un elemento chiave nell'intero framework oltre a
