...
diverse ottimizzazioni che la JVM e l'hardware~\cite{jmh:details} sottostante sono in grado di fare quando il benchmark
esegue solo il componente, ma tali ottimizzazioni non possono essere effettuate quando il componente è eseguito come
parte dell'intera applicazione, producendo dati falsati.
...
\begin{itemize}
    \item \texttt{@Warmup}: si specifica quante iterazioni devono essere eseguite ``a vuoto'' prima di effettuare gli effettivi test;
    ...
    loop) e quindi \textit{JIT compiled}~\cite{jmh:details}.
...
Di seguito sono analizzati i risultati ottenuti dai benchmark dei \System.
...
grazie all'utilizzo delle \texttt{CList} (descritte in dettaglio in~\ref{sec:lista-di-componenti}) verrà verificato a tempo di compilazione che i
