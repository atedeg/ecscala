...
object ExcludingSystemBuilder {
  def apply[L <: CList: CListTag, E <: CList: CListTag]: ExcludingSystemBuilder[L, E] = BuilderUtils.ExcludingSystemBuilderImpl()
}
...
private object BuilderUtils {
  abstract class BaseSystemBuilder[L <: CList: CListTag](
    ...
    def builderConstructor(before: (DeltaTime, World, View[L]) => Unit, after: (DeltaTime, World, View[L]) => Unit, precondition: => Boolean): BuilderType

    override def withBefore(newBefore: (DeltaTime, World, View[L]) => Unit) = builderConstructor(newBefore, after, precondition)
    override def withAfter(newAfter: (DeltaTime, World, View[L]) => Unit) = builderConstructor(before, newAfter, precondition)
  ...
  class SystemBuilderImpl[L <: CList: CListTag](
    ...
    precondition: => Boolean = true
  ) extends BaseSystemBuilder[L](beforeHandler, afterHandler, precondition) with SystemBuilder[L] {
  ...
  class ExcludingSystemBuilderImpl[L <: CList: CListTag, E <: CList: CListTag](
    ...
    afterHandler: (DeltaTime, World, View[L]) => Unit = (_, _, _: View[L]) => (),
    precondition: => Boolean = true
  ) extends BaseSystemBuilder[L](beforeHandler, afterHandler, precondition) with ExcludingSystemBuilder[L, E] {
    ...
    override def systemConstructor(f: (Entity, L, DeltaTime, World, View[L]) => Deletable[L]) = new ExcludingSystem[L, E] {
      override def update(e: Entity, c: L)(dt: DeltaTime, w: World, v: View[L]) = f(e, c, dt, w, v)
      override def before(dt: DeltaTime, w: World, v: View[L]): Unit = beforeHandler(dt, w, v)
      override def after(dt: DeltaTime, w: World, v: View[L]): Unit = afterHandler(dt, w, v)
      override def shouldRun = precondition
